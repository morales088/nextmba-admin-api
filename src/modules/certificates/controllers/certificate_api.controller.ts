...
export class CertificateApiController {
  ...
  async generateCertificate(
    ...
    const htmlFilePath =
      studCertificate.certificate_tier == 1
        ? 'src/common/templates/course-certificate.template.html'
        : 'src/common/templates/module-certificate.template.html';
    ...
    console.log(studCertificate.certificate_tier == 1);
    console.log(studcourse.starting_date <= new Date('2024-07-31'));

    const template =
      studCertificate.certificate_tier == 1 && studcourse.starting_date <= new Date('2024-07-31')
        ? attendanceCert
  ...
  formatLectures(speakers: any[]) {
    ...
    return hasAppliedStudies ? `${monthlyTalk}<br><h1>Applied Studies:</h1>${appliedStudies}` : monthlyTalk;
  }
